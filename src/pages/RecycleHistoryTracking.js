...
function RecycleHistoryTracking() {
  ...
  return (
    <>
      <Header />
      <Box sx={{ padding: 4 }}>
        <Paper sx={{ padding: 2, marginBottom: 3 }} style={{marginTop:"50px"}}>
          <Typography variant="h6" gutterBottom>
            Search
          </Typography>
          <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
              ...
              {filteredData.map((item, index) => (
                <Grid item xs={12} sm={6} md={4} key={index}>
                  ...
                  >
                    ...
                    <Box>
                      <Typography variant="subtitle1" gutterBottom>
                        <strong style={{color:"#81b4d3"}}>Name:</strong> {item.recycleName}
                      </Typography>
                      <Typography variant="body2" gutterBottom>
                        <strong style={{color:"#81b4d3"}}>Description:</strong> {item.description}
                      </Typography>
                      <Typography variant="body2" gutterBottom>
                        <strong style={{color:"#81b4d3"}}>Date:</strong> {item.recycleDate}
                      </Typography>
                      <Typography variant="body2" gutterBottom>
                        <strong style={{color:"#81b4d3"}}>Location:</strong> {item.recycleLocation}
                      </Typography>
                      <Typography variant="body2">
                        <strong style={{color:"#81b4d3"}}>Status:</strong> {item.productStatus}
                      </Typography>
                    </Box>
                  </Box>
                </Grid>
      ...
      {selectedItem && (
        <Dialog open={true} onClose={handleCloseDialog}>
          ...
          <DialogContent>
            <img
              src={selectedItem.imagesUrl || defaultImage}
              alt={selectedItem.recycleName}
              style={{ width: "100%", height: "auto", marginBottom: "16px" }}
            />
            <Typography variant="subtitle1" gutterBottom>
              <strong>Recycle Name:</strong> {selectedItem.recycleName}
            </Typography>
            <Typography variant="body2" gutterBottom>
              <strong>Description:</strong> {selectedItem.description}
            </Typography>
            <Typography variant="body2" gutterBottom>
              <strong>Date:</strong> {selectedItem.recycleDate}
            </Typography>
            <Typography variant="body2" gutterBottom>
              <strong>Location:</strong> {selectedItem.recycleLocation}
            </Typography>
            <Typography variant="body2">
              <strong>Status:</strong> {selectedItem.productStatus}
            </Typography>
          </DialogContent>
          <DialogActions>
